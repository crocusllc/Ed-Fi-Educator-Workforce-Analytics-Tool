...
STAFF_ASSIGNMENT_BASE AS (
    ...
    SELECT
        ...
        CASE
            ...
            ELSE t.EndDate
        END AS EndDate,
        -- Calculate the start year of the school year based on StartDate (June 30th to July 1st)
        CASE
        ...
        -- Calculate the end year of the school year based on EndDate (June 30th to July 1st))
        --Ensure that the SchoolYearEnd field is never NULL by filling current Year.  
        CASE
            WHEN MONTH(t.EndDate) >= 7 AND t.EndDate is not null THEN YEAR(t.EndDate)
            WHEN MONTH(GETDATE()) >= 7 AND t.EndDate is null THEN YEAR(GETDATE())
            WHEN MONTH(t.EndDate) < 7 AND t.EndDate is not null THEN YEAR(t.EndDate)-1
            WHEN MONTH(GETDATE()) < 7 AND t.EndDate is null THEN YEAR(GETDATE())-1
...
SELECT
   ...
   CAST(sye.SchoolYearStart+1  AS NVARCHAR(4)) + '-' + CAST(sye.SchoolYearStart + 2 AS NVARCHAR(4)) AS RetainedSchoolYear,
    ...
    CASE WHEN cred.ShortDescription IS NULL THEN 'None' ELSE cred.ShortDescription END  AS CredentialType,
    ...
    CASE WHEN scdesc.CodeValue IS NULL THEN 'District' ELSE scdesc.CodeValue END  AS SchoolSegment,
    ...
    s.BirthDate,
    CASE
          WHEN YEAR(seoaa.BeginDate) = sye.SchoolYearStart  AND  MONTH(seoaa.BeginDate) >= 7 THEN 'New Hire'
        WHEN YEAR(seoaa.BeginDate) = sye.SchoolYearStart + 1 AND  MONTH(seoaa.BeginDate) < 7 THEN 'New Hire'
        ELSE NULL
    END AS NewHire,
    CASE
        WHEN (CONVERT(int,CONVERT(char(8),DATEFROMPARTS(sye.SchoolYearStart+1,7,1),112))-CONVERT(char(8),s.BirthDate,112))/10000 >= 56 THEN 'Near Retirement'
        ELSE NULL
